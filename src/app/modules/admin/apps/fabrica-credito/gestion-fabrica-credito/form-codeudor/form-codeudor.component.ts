...
export class FormCodeudorComponent implements OnInit {
    ...
    public addValidation() {
        ...
        this.form.get('ocupacion').valueChanges.subscribe((e: string) => {
            if (e !== 'EPLDO' && !(e === 'INDEFO' || e === 'PROIN' || e === 'INDNFO')) {
            ...
            if (e === 'EPLDO') {
                ...
                this.form
                    ...
                    ?.setValidators([Validators.required, this.validatedDate.bind(this)]);
                this.form
                    .get('fechaIngresoFabrica')
                    ?.enable({ emitEvent: true, onlySelf: true });
            } else {
    ...
    public onPostDatos(): void {
        ...
        const fechaIngresoFabricaFormato = fechaIngresoFabrica ? moment(fechaIngresoFabrica.toString()).format('YYYY-MM-DD') : "0099-01-01";
        const fechaDesvinculacionFormato = fechaDesvinculacion ? moment(fechaDesvinculacion.toString()).format('YYYY-MM-DD') : "0099-01-01";
        const fechaVinculacionFormato = fechaVinculacion ? moment(fechaVinculacion.toString()).format('YYYY-MM-DD') : "0099-01-01";
