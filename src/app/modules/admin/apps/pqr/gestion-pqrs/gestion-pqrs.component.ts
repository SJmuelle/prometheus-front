...
export class GestionPQRSComponent implements OnInit {
  ...
  seguimiento: { idPqrs: any; idPqrsPadre: number; motivoRechazo: string; idTipoComentario: number; detalle: string; };
  ...
  buscarDatos() {
    ...
    this._pqrService
      .getListados(url)
      .subscribe((response: any) => {
        this.aprobarComentario = response.aprobarComentario == 1 ? true : false;
      });
        ...
        if (response) {
          if (response.status == 200) {
            if (response.data.respuesta == 'OK') {
              ...
              let url = `/notificacion-crear-pqrs-respuesta/${data.idPqrs}/${data.respuesta == true ? 1 : 0}/${data.comentario}`;
