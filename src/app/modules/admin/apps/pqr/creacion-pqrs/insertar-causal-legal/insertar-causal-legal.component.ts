...
export class InsertarCausalLegalComponent implements OnInit {
        ...
        this._pqrService.getListados(url).subscribe((response: any) => {
            if (response.length != 0) {
                ...
                if (variable == 'listadoResponsablePQRS') {
                    ...
                    let index = this.listadoProcedimientosPQRS.findIndex(
                        (data) => data.id == tipo
                    );
                    if (index != -1) {
                        this.datos.fechaParaSolucion =
                            this.listadoProcedimientosPQRS[index].diaSolucion;
                    } else {
                        this.datos.fechaParaSolucion = '';
                    }
                }
    ...
    nombreSolucion(dato) {
        ...
        let retu;
        if (index != -1) {
            retu = this.listadoProcedimientosPQRS[index].descripcion;
        } else {
            retu = '';
        }
        return retu;
